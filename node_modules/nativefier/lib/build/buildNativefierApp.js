...
function trimUnprocessableOptions(options) {
    if (options.packager.platform === 'win32' &&
        ...
        log.warn(`*Not* setting [${optionsPresent.join(', ')}], as couldn't find Wine.`, 'Wine is required when packaging a Windows app under on non-Windows platforms.');
...
async function buildNativefierApp(rawOptions) {
    ...
    await buildIcon_1.convertIconIfNecessary(options);
